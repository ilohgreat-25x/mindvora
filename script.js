...
window.DeviceResponsive = (function() {
  ...
  function updateDeviceInfo() {
    ...
    deviceInfo.type = detectDeviceType(window.innerWidth);
    ...
    return {
      changed: prevType !== deviceInfo.type || prevOrientation !== deviceInfo.orientation,
      typeChanged: prevType !== deviceInfo.type,
      orientationChanged: prevOrientation !== deviceInfo.orientation
    };
  }
  ...
  function storeDeviceInfo(userEmail) {
    ...
    var storageKey = 'mv_device_' + userEmail;
    var deviceData = {
  ...
  function onWindowResize() {
    ...
    if (change.changed) {
      document.dispatchEvent(new CustomEvent('deviceChanged', { detail: deviceInfo }));
    }
  }
  ...
  function onOrientationChange() {
    ...
    if (change.orientationChanged) {
      document.dispatchEvent(new CustomEvent('orientationChanged', { detail: deviceInfo }));
    }
  }
  ...
  function init() {
    ...
    if (window.matchMedia) {
      ...
      mobileQuery.addEventListener('change', onWindowResize);
      tabletQuery.addEventListener('change', onWindowResize);
      desktopQuery.addEventListener('change', onWindowResize);
    }
  ...
  return {
    ...
    getStoredDeviceInfo: getStoredDeviceInfo,
    isMobile: isMobile,
    isTablet: isTablet,
    isDesktop: isDesktop,
...
// Initialize device responsive when DOM is ready
...
var MindvoraRT = (function() {
  ...
  var peerConnections = {}; // viewerUid → RTCPeerConnection
  ...
  function handleMessage(msg) {
    switch(msg.type) {
      ...
      case 'ERROR':
        if (typeof showToast === 'function') showToast('⚠️ ' + (msg.message || 'Stream error'));
        break;
    }
  }
  ...
  function onLiveStarted(msg) {
    ...
    if (typeof db !== 'undefined' && typeof state !== 'undefined' && state.user) {
      db.collection('notifications').add({
        to: state.user.uid,
        type: 'live_started',
        text: (msg.hostName || 'Someone') + ' just went LIVE! 🔴',
        hostUid: msg.hostUid,
  ...
  function onViewerJoined(msg) {
    ...
    pc.createOffer().then(function(offer) {
      ...
      send({
        ...
        fromUid: state && state.user ? state.user.uid : '',
        toUid: uid,
  ...
  function stopLocalStream() {
    ...
    var liveVideo = document.getElementById('live-local-video');
  ...
  function showLiveBanner(msg) {
    ...
    if (!banner) {
      ...
      banner.style.cssText = [
        ...
        'box-shadow:0 8px 32px rgba(22,101,52,.6);z-index:9999;',
        'display:flex;align-items:center;gap:12px;font-size:14px;',
        'animation:slideDown .3s ease;cursor:pointer;max-width:340px;'
      ].join('');
      document.body.appendChild(banner);
    }

    var avatar = msg.hostAvatar
      ? '<img src="'+msg.hostAvatar+'" style="width:36px;height:36px;border-radius:50%;object-fit:cover;border:2px solid #fff">'
      : '<div style="width:36px;height:36px;border-radius:50%;background:#0a1a0f;display:flex;align-items:center;justify-content:center;font-size:18px">🎥</div>';

    banner.innerHTML =
      '<span style="background:#ef4444;color:#fff;font-size:10px;font-weight:700;padding:2px 7px;border-radius:6px;letter-spacing:.5px">LIVE</span>' +
      avatar +
      '<div><div style="font-weight:700;font-size:13px">'+(msg.hostName||'Someone')+' is LIVE</div>' +
      '<div style="font-size:11px;opacity:.85">Tap to join the stream</div></div>';

    banner.onclick = function() {
      if (typeof openLiveView === 'function') openLiveView(msg);
  ...
  function hideLiveBanner(roomId) {
    var banner = document.getElementById('live-notification-banner');
    if (banner && banner.dataset.roomId === roomId) {
  ...
  function updateLiveUI(status, roomId) {
    ...
    var liveCard = document.getElementById('live-host-card');
    if (status === 'started') {
   ...
   */
  function openGoLive() {
    if (!state || !state.user) { showToast('Please log in first.'); return; }
    if (isLiveHost) { showToast('You are already live!'); return; }

    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      showToast('❌ Your browser does not support live streaming.');
      return;
    }
    ...
    navigator.mediaDevices.getUserMedia({ video: true, audio: true })
      .then(function(stream) {
        ...
        send({
          type:   'LIVE_START',
          uid:    state.user.uid,
          name:   (state.profile && state.profile.displayName) || state.user.displayName || 'Creator',
          avatar: (state.profile && state.profile.photoURL)    || state.user.photoURL    || ''
        });
  ...
  /**
   * endLive() — called when host clicks "End Stream"
   */
  function endLive() {
    if (!isLiveHost || !currentRoomId) return;
    send({ type: 'LIVE_END', roomId: currentRoomId });
    stopLocalStream();
    isLiveHost    = false;
    currentRoomId = null;
    updateLiveUI('ended', null);
    if (typeof showToast === 'function') showToast('Stream ended.');
  }

  /**
   * joinLiveStream(roomId) — called when a viewer wants to watch a live stream
   */
  function joinLiveStream(roomId) {
    if (!state || !state.user) { showToast('Please log in to watch.'); return; }
    currentRoomId = roomId;
    send({
      type:   'JOIN_ROOM',
      roomId: roomId,
      uid:    state.user.uid,
      name:   (state.profile && state.profile.displayName) || state.user.displayName || 'Viewer'
    });
  }

  /**
   * openLiveView(msg) — open the live viewer modal
   */
  function openLiveView(msg) {
    var modal = document.getElementById('modal-live-view');
    if (!modal) return;
    var nameEl = modal.querySelector('.live-host-name');
    if (nameEl) nameEl.textContent = (msg.hostName || 'Creator') + ' — LIVE 🔴';
...
console.log('[Mindvora] Initializing Firebase...');
try {
  ...
  console.log('[Mindvora] Firebase initialized successfully');
} catch(e) {
  console.error('[Mindvora] Firebase initialization error:', e);
}
...
auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL).then(function(){
  console.log('[Mindvora] Persistence set successfully, registering auth listener');
  initAuthListener();
}).catch(function(err){
  console.error('[Mindvora] Persistence error:', err);
  console.log('[Mindvora] Attempting to register auth listener anyway');
  initAuthListener();
});
...
function loadPaystack() {
  ...
  _psPromise = new Promise(function(resolve, reject) {
    ...
    s.onload = function() {
      ...
      var iv = setInterval(function() {
        tries++;
        if (typeof PaystackPop !== 'undefined') { clearInterval(iv); resolve(); }
        else if (tries > 20) { clearInterval(iv); reject(new Error('paystack-timeout')); }
      }, 250);
    };
    s.onerror = function() { _psPromise = null; reject(new Error('paystack-load-failed')); };
    document.head.appendChild(s);
  });
  return _psPromise;
}

// Convert a USD price to Paystack NGN kobo (1 USD ≈ 1600 NGN)
function usdToNGNKobo(usd) {
...
function payWithPaystack(opts) {
  ...
  var amount = opts.amount;            // already in kobo (NGN)
...
function doLogin(){
  ...
  if (!isEmail) {
    ...
    loginPromise = db.collection('users').where('handle', '==', email).limit(1).get()
      .then(function(snap) {
        ...
        console.log('[Mindvora] Found email for username:', userEmail);
        return auth.signInWithEmailAndPassword(userEmail, pass);
      });
  } else {
    console.log('[Mindvora] Email login detected');
  ...
  loginPromise
    ...
    .catch(function(e) {
      console.error('[Mindvora] Login error:', e.code, e.message);
...
function doRegister(){
  ...
  if(phone && phone.replace(/[^\d]/g,'').length < 7){ err.textContent='Enter a valid phone number (with country code).'; return; }
    ...
    try {
      ...
      .then(function(data) {
        if (data && data.status) {
          ...
          showToast('❌ ' + ((data && data.message) || 'Could not send email.'));
          reject(new Error((data && data.message) || 'OTP send failed'));
        }
      })
      .catch(function(error) {
        console.error('OTP email error:', error);
        showToast('❌ Email service unreachable. Try again.');
        reject(error);
      });
    } catch (e) {
...
// Send SMS OTP via Twilio (code is generated by Twilio server-side)
function sendPhoneOTP(phone) {
  return new Promise(function(resolve, reject) {
    ...
    .then(function(data) {
      ...
      if (data && data.status) {
        otpState.phone = phone.trim();
        showToast('📱 SMS code sent to ' + phone.trim() + '!');
        startOTPTimer();
        resolve();
      } else {
        showToast('❌ ' + ((data && data.message) || 'Could not send SMS.'));
        reject(new Error((data && data.message) || 'SMS send failed'));
      }
    })
    .catch(function(error) {
      if (btn) { btn.disabled = false; btn.textContent = 'Send Code'; }
      showToast('❌ SMS service unreachable. Try again.');
      reject(error);
    });
  });
}

// Verify the SMS code with Twilio backend
function verifyPhoneOTP(phone, code) {
  return new Promise(function(resolve, reject) {
    if (!phone || !code || String(code).length < 4) {
      showToast('❌ Enter the code you received by SMS.');
      reject(new Error('Invalid code'));
      return;
    }
    fetch(BACKEND_URL + '/api/otp/verify-sms', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ phone: phone.trim(), code: String(code) })
    })
    .then(function(r){ return r.json(); })
    .then(function(data) {
      if (data && data.status) {
...
function showOTPScreen(email) {
  ...
  if (!otpScreen) {
    console.error('OTP screen element not found in HTML');
    return;
  }
...
function verifyUserEmail() {
  if (!otpState.userId || !auth.currentUser) {
  ...
  db.collection('users').doc(userId).update({
    ...
    if (phoneCard && phoneInput && (state.profile.phone || otpState.pendingPhone)) {
      if (state.profile.phone) phoneInput.value = state.profile.phone;
      else if (otpState.pendingPhone) phoneInput.value = otpState.pendingPhone;
...
// Hide OTP screen + mount the app (called after email, or phone, verification)
function finishEmailVerification() {
  var otpScreen = document.getElementById('otp-screen');
  if (otpScreen) {
    otpScreen.style.display = 'none';
    otpScreen.style.visibility = 'hidden';
...
// Send the SMS code from the OTP screen (uses the phone typed on the card)
function sendPhoneOTPFromScreen() {
  var input = document.getElementById('phone-otp-input');
  var phone = input ? input.value.trim() : '';
  if (!phone) { showToast('❌ Enter your phone number first.'); if (input) input.focus(); return; }
  otpState.pendingPhone = phone;
  sendPhoneOTP(phone).then(function() {
    var codeInput = document.getElementById('phone-otp-code');
    if (codeInput) codeInput.focus();
  }).catch(function(){});
}

// Verify the SMS code from the OTP screen, then finish setup
function verifyPhoneOTPFromScreen() {
  var input = document.getElementById('phone-otp-input');
  var codeInput = document.getElementById('phone-otp-code');
  var phone = input ? input.value.trim() : (otpState.pendingPhone || otpState.phone || '');
  ...
  verifyPhoneOTP(phone, code).then(function() {
    ...
    showToast('✅ Phone verified! Welcome to Mindvora 🌿');
    finishEmailVerification();
  }).catch(function(){});
}

// Skip phone verification for now (email already verified)
function skipPhoneOTP() {
  var phoneCard = document.getElementById('otp-phone-card');
  if (phoneCard) phoneCard.style.display = 'none';
  if (otpState.pendingPhone && otpState.userId) {
    db.collection('users').doc(otpState.userId).update({ phone: otpState.pendingPhone }).catch(function(){});
  }
  finishEmailVerification();
}
...
function resendOTP() {
  ...
  if (!otpState.email || !otpState.userId) {
    showToast('❌ Resend failed. Please sign up again.');
    return;
  }
...
auth.onAuthStateChanged(function(user){
  console.log('[Mindvora] Auth state changed. User:', user ? 'logged in (UID: '+user.uid+')' : 'logged out');
  if(user){
    state.user=user;
    ...
    db.collection('users').doc(user.uid).get().then(function(snap){
      ...
      clearTimeout(_mountTimeout);
      _mountFired = true;
      console.error('[Mindvora] Firestore error during profile fetch:', err);
      state.profile={name:user.displayName||user.email,handle:(user.email||'user').split('@')[0],color:COLORS[0],plan:'free'};
      console.log('[Mindvora] Using fallback profile, calling mountApp()');
      mountApp();
    });
  } else {
    ...
    if(appSc)  { 
      appSc.style.setProperty('display', 'none', 'important');
      appSc.style.visibility='hidden'; 
...
function mountApp(){
  console.log('[Mindvora] mountApp() called');
  if(!state.profile) {
    console.warn('[Mindvora] mountApp called but state.profile is null!');
    return;
  }
  try {
  ...
  if(authSc) {
    ...
    console.log('[Mindvora] Auth-screen hidden');
  } else {
    console.warn('[Mindvora] auth-screen element not found!');
  }
  if(appSc) {
    appSc.style.setProperty('display', 'flex', 'important');
    appSc.style.visibility = 'visible';
    appSc.style.zIndex = '1';
    appSc.style.opacity = '1';
    console.log('[Mindvora] App-screen shown');
  } else {
    console.error('[Mindvora] app-screen element not found!');
  }
...
document.getElementById('btn-submit-paid').addEventListener('click', function(){
  ...
  if(!state.user) return;
  payWithPaystack({
    amount: usdToNGNKobo(adState.budget), currency:'NGN',
    ref:'ZAD-'+Date.now(),
    onSuccess:function(r){
      db.collection('ads').add({
...
function openCreatorSub(creatorId,creatorName,price){
  ...
  payWithPaystack({
    amount: usdToNGNKobo(price), currency:'NGN',
...
function openGoLive() {
  ...
  document.getElementById('live-err').innerHTML = '';
  document.getElementById('live-title-inp').value = '';
  _requestLiveCamera();
}

function _requestLiveCamera() {
  ...
  navigator.mediaDevices.getUserMedia({ video: true, audio: true })
    ...
    .catch(function(err) {
      ...
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
        msg = '⚠️ Camera/mic access denied. Please allow access in your browser settings, then ';
      } else if (err.name === 'NotFoundError' || err.name === 'DevicesNotFoundError') {
        msg = '⚠️ No camera or microphone found on this device. ';
      } else if (err.name === 'NotReadableError') {
        msg = '⚠️ Camera/mic is already in use by another app. Close it and ';
      } else {
        msg = '⚠️ Could not access camera/mic (' + (err.message || err.name) + '). ';
      }
      errEl.innerHTML = msg +
        '<span style="color:#ef4444;cursor:pointer;text-decoration:underline;font-weight:700" ' +
        'onclick="_requestLiveCamera()">tap to retry</span>.';
    });
}
...
function buyItem(id,name,price,sellerEmail){
  ...
  payWithPaystack({amount:usdToNGNKobo(price),currency:'NGN',ref:'ZM-'+Date.now(),metadata:{itemId:id,itemName:name,buyerId:state.user.uid,sellerEmail:sellerEmail},onSuccess:function(r){
...
window.addEventListener('beforeinstallprompt', function(e){
  ...
  setTimeout(function(){
    ...
    showToast('📲 Install Mindvora on your device for the full app experience!');
  }, 12000);
});
window.addEventListener('appinstalled', function(){
  deferredPrompt = null;
  installPromptShown = true;
  showToast('✅ Mindvora installed! Enjoy! 🌿');
});
window.addEventListener('install-prompt-request', function(){
  if (!deferredPrompt) { showToast('⚠️ Your browser does not support installation. Open via Chrome on Android or Safari on iPhone.'); return; }
